...
if [[ "$slug" == "apache/incubator-dubbo" ]]; then
    sudo chown -R $USER .
    mvn clean install -DskipTests -Dmaven.repo.local=$AZ_BATCH_TASK_WORKING_DIR/input/dependencies ${MVNOPTIONS} |& tee mvn-install.log
    ...
    mvn clean install -am -pl $module -DskipTests -Dmaven.repo.local=$AZ_BATCH_TASK_WORKING_DIR/input/dependencies ${MVNOPTIONS} |& tee mvn-install.log
...
cp $projectname.zip $AZ_BATCH_TASK_WORKING_DIR/input
cd ~/$slug
