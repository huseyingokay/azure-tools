...
if [[ "$slug" == "apache/incubator-dubbo" ]]; then
    sudo chown -R $USER .
    mvn test -Dsurefire.timeout=5 ${MVNOPTIONS} |& tee mvn-test.log
    ...
    mvn test -am -pl $module -Dsurefire.timeout=5 ${MVNOPTIONS} |& tee mvn-test.log
...
grep "The forked VM terminated without saying properly goodbye. VM crash or System.exit called" mvn-test.log
ret=${PIPESTATUS[0]}
echo "mvn test -am -pl $module -Dsurefire.timeout=5 ${MVNOPTIONS} |& tee mvn-test.log" |& tee mvn-test-command.sh
if [[ $ret == 0 ]]; then
	cd /mnt/batch/tasks/workitems/SUA_tmp_r2_7M21d18h23m34s/job-1
	zip -rq $modified_slug_module.zip ${slug%/*} /$slug/mvn-test.log
